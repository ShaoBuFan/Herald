...
**版本说明（2026-09-21 更新）**：本文初版只依赖 arXiv 摘要页；随后取得七篇 PDF 正文（`docs/ref/`），本文据此做了三类修正——补入 SayCan 的精确公式与消融数字、补入 WebGPT 的完整动作空间、**纠正了初版「ReAct 未引 Scratchpads」的错误结论**。第 9 节逐条标注了当前的证据层级。

**注意版本陷阱**：手上的 PDF 版本普遍晚于原始提交（CoT 差近一年），引用边只对特定版本成立。见第 9 节第一小节。

> **阅读指南**：[`paper-reading-map.md`](paper-reading-map.md) 给出论文依赖图、推荐阅读顺序，以及哪几篇尚未取到 PDF。本文是内容分析，那份是阅读路径。
...
### 一条前提性说明：本文引用的版本晚于原始提交

引用边与部分数字取自**手上的 PDF 版本**，而这些版本可能比原始提交晚数月到一年。逐篇核对（各 PDF 页眉）：

| 论文 | 原始提交 | PDF 版本 | 差多久 |
|---|---|---|---|
| WebGPT | 2021-12-17 | v3，2022-06-01 | 约 6 个月 |
| CoT | 2022-01-28 | **v6，2023-01-10** | **近一年** |
| Self-Consistency | 2022-03-21 | v4，2023-03-07 | 近一年 |
| SayCan | 2022-04-04 | v2，2022-08-16 | 约 4 个月 |
| Zero-shot CoT | 2022-05-21 | v4，2023-01-29 | 约 8 个月 |
| Inner Monologue | 2022-07-12 | v1（单版本） | — |
| ReAct | 2022-10-06 | v3，2023-03-10 | 约 5 个月 |

**具体后果**：CoT v6 的参考文献表里含 2022-03 的 Self-Consistency 与 2022-04 的 SayCan，这是 v1 不可能有的。所以「CoT 引用了谁」只对 v6 成立。**判断思想影响关系时用原始提交日期，不要用引用边。** 完整的图与逐条边见 [`paper-reading-map.md`](paper-reading-map.md)。
