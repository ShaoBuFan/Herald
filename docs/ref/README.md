...
本目录放两样东西：调研时用到的论文 PDF，以及从它们里面摘出的**原文引语抄本**。
...
1. **引语必须带版本号。** 引文与页码只在特定版本上成立（见下节「为什么固定版本号」）。引语后面的出处要写成 `docs/ref/2210.03629v3.pdf` §3.2 这样的形式。
2. **引语只允许有一个位置，就是这里。** `docs/research/` 里引用它，不复制它。同一句引语出现在两处，迟早会不一致。

## 谁写引语

**引语进 `ref/` 的前提：作者在阅读时把它标记出来了。**

**AI 不主动往这里放引语。** 判断「哪句原文重要」是阅读理解的一部分，属于 `docs/notes/` 该做的判断，不是参考件该做的。AI 在这里能做的只有一件事：

> **把作者已标记的引语，逐字回原文核准，并规范化出处格式。**

核准的意思是回 PDF 逐字比对，而不是从提取出的文本里复制——`pdftotext` 会改掉排版、花体字和连字符，详见本文末尾。

## PDF 不入 git

PDF 是第三方版权材料，体积大且可重新下载，入库只会让仓库膨胀（见根目录 `.gitignore`）。**引语抄本是 Markdown，入库。**

收录内容与下载命令见下。`docs/research/` 引用本目录文件时，写作 `docs/ref/<文件名>`。
